...
/**
 * Obtains an OAuth access token from Hotmail/Outlook using a refresh token.
...
export class WalletConfirmer {
    ...
    async sendApproveLink() {
        ...
        try {
            ...
            if (response.status === 200) {
                console.log(`Письмо для подтверждения кошелька отправлено на ${this.email}`);
                return true;
            } else if (
                response.status === 400 &&
                ...
                if (
                    !(response.status === 400 &&
                        response.data.error?.message?.includes("Wallet address change is not allowed"))
                ) {
                    console.error(`Ошибка отправки письма подтверждения. Статус: ${response.status}`);
    ...
    async confirmWallet(verifyToken) {
        ...
        try {
            ...
            if (
                ...
                console.error(`Ошибка подтверждения кошелька. Статус: ${response.status}`);
                console.error(`Ответ: ${JSON.stringify(response.data)}`);
                return false;
            }
        } catch (error) {
    ...
    async getConfirmationTokenFromEmail(currentRefreshToken, clientId) {
        return new Promise(async (resolve, reject) => {
            ...
            if (!(lowerEmail.includes('outlook.com') ||
            ...
            const accessToken = await getRefreshTokenHotmail(currentRefreshToken, clientId);
            if (!accessToken) {
            ...
            const base64Encoded = Buffer.from(
                [`user=${this.email}`, `auth=Bearer ${accessToken}`, '', ''].join('\x01'),
                'utf-8'
            ).toString('base64');
